...
app = FastAPI(title="GovScheme-Env", version="1.0.0")

DATA_DIR = Path(__file__).parent
env = GovSchemeEnvironment(
    schemes_path=str(DATA_DIR / "schemes.json"),
    citizens_path=str(DATA_DIR / "citizens.json"),
)
...
function getProfile() {
  return {
    ...
    income: parseInt(document.getElementById('f-income').value)||0,
    land: parseFloat(document.getElementById('f-land').value)||0,
    house: document.getElementById('f-house').value,
    aadhaar: document.getElementById('aadhaar').checked,
    bank: document.getElementById('bank').checked,
    lpg: document.getElementById('lpg').checked,
    govt: document.getElementById('govt').checked,
    tax: document.getElementById('tax').checked,
    prof: document.getElementById('prof').checked
  };
}
...
function isEligible(p, e) {
  ...
  if(e.caste_or_female) {
    if(!['SC','ST'].includes(p.caste) && p.gender !== 'Female') return false;
  }
  return true;
}
...
function runCheck() {
  ...
  setTimeout(() => {
    ...
    if(ranked.length === 0) {
      html = '<div style="text-align:center;padding:32px 0;color:#aaa;font-size:13px">No eligible schemes found. Try adjusting the profile.</div>';
    } else {
      const total = ranked.reduce((s,x) => s + x.benefit, 0);
      html += '<div class="result-header">';
      html += '<div class="result-count">' + ranked.length + ' schemes found</div>';
      html += '<div class="result-sub">Total potential benefit: Rs ' + total.toLocaleString('en-IN') + '/year</div>';
      ...
      ranked.forEach((s,i) => {
        ...
        html += '<div class="scheme-card' + (isTop ? ' rank1' : '') + '">';
        html += '<div class="sc-top"><div class="sc-name"><span class="' + rnCls + '">' + (i+1) + '</span>' + s.name + '</div>';
        html += '<span class="' + pillCls + '">' + benefitText + '</span></div>';
        html += '<div class="sc-ministry">' + s.ministry + '</div>';
        html += '<div class="sc-desc">' + s.desc + '</div>';
        if(s.benefit > 0) {
          html += '<div class="sc-bar"><div class="bar-track"><div class="bar-fill" style="width:' + barPct + '%;background:' + barColor + '"></div></div>';
          html += '<span class="bar-label">' + barPct + '%</span></div>';
        }
        html += '</div>';
      });
    }
    ...
    btn.disabled = false;
    btn.textContent = 'Find eligible schemes';
  }, 600);
}
...
def reset(body: Dict[str, Any] = {}):
    task_name = body.get("task_name", "scheme_identification")
    citizen_id = body.get("citizen_id", None)
    seed = body.get("seed", None)
    result = env.reset(task_name=task_name, citizen_id=citizen_id, seed=seed)
    return result
...
@app.get("/schemes")
def schemes():
    schemes_data = json.loads((DATA_DIR / "schemes.json").read_text())
    return {"schemes": schemes_data, "count": len(schemes_data)}


@app.get("/citizens")
def citizens():
    citizens_data = json.loads((DATA_DIR / "citizens.json").read_text())
    return {"count": len(citizens_data), "citizen_ids": [c["citizen_id"] for c in citizens_data]}